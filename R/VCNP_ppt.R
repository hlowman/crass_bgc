...
ppt_RSA$ID = "RSA"
ppt_RSA = tibble::add_column(ppt_RSA, station = "avg_valletoledo_losposos", .after = "datetimeMT")
...
ppt_SULF$ID = "SULF"
ppt_SULF = tibble::add_column(ppt_SULF, station = "avg_redondo_sanantonio", .after = "datetimeMT")
...
ppt_EFJ$ID = "EFJ"
ppt_EFJ = tibble::add_column(ppt_EFJ, station = "avg_losposos_redondo_vallegrande", .after = "datetimeMT")
...
for(i in names(pptz_m)){
  pptz_m[[i]]$datetimeMT = lubridate::round_date(pptz_m[[i]]$datetimeMT, "month") 
  pptz_m[[i]] = 
    pptz_m[[i]] %>% 
...
# plot 
ppt_cum_m_p =
  ggplot(data = ppt_cum_m, aes(x=datetimeMT, y=cumulative_precip_mm)) +
